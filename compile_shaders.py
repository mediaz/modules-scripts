...
if len(sys.argv) < 2:
    print("Usage: compile_shaders.py tools_dir")
    exit(1)

tools_dir = sys.argv[1]

def embed_binary(filepath):
    result = run([f"{path}/{tools_dir}/{platform.system()}/bin2header.exe", filepath], stdout=PIPE, stderr=PIPE, universal_newlines=True)
...
def compile_to_spv(filepath):
    ...
    re = run([f"{path}/{tools_dir}/{platform.system()}/glslc", "-o",  f"{filepath}_.spv", filepath], stdout=stdout, stderr=stderr, universal_newlines=True)
    if re.returncode != 0:
        logger.error(f"Failed to compile {filepath}")
        return re.returncode
    else:
        re = run([f"{path}/{tools_dir}/{platform.system()}/spirv-opt", "-O", "-o",  f"{filepath}.spv", f"{filepath}_.spv"], stdout=stdout, stderr=stderr, universal_newlines=True)
